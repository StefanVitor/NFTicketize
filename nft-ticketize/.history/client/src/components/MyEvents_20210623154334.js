...
function ButtonsBox({ onReloadList, onSetOverlay}) {
  ...
  const createEvent = async () => {
    const NFTicketizeContractArt = require( "../contracts/NFTicketize.json"); // for Hardhat

    const storageContract = new web3.eth.Contract(NFTicketize, );

    const daiContract = new ethers.Contract(EnvConstant.contractAddress,NFTicketizeContractArt, provider);

    //Insert into smart contract
    var params = [
      {
        from: window.ethereum.selectedAddress,
        to: EnvConstant.contractAddress,
        gas: '0x76c0', // 30400
