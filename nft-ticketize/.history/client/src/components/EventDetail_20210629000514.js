...
function TicketCategoryDetail({onReloadList, onSetOverlay}) {  
  ...
  const buyDialogHandleBuy = async() => {
    ...
    try {
      ...
      const tokenId = await fetch(`https://api-dev.rarible.com/protocol/v0.1/ethereum/nft/collections/` + EnvConstant.contractAddress +`/generate_token_id?minter=`+ address + ``);
    }
    catch (err) {  
      console.log(err);
      onReloadList(true);
