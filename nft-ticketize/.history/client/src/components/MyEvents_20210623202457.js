...
import { NFTicketize } from "../contracts/NFTicketize.json";
...
const provider = new ethers.providers.Web3Provider(window.ethereum); // for ethers.js

const NFTicketizeContractArt = require( "../contracts/NFTicketize.json"); // for ethers.js
...
function ButtonsBox({ onReloadList, onSetOverlay}) {
  ...
  const createEvent = async () => {
    ...
    const daiContract = new ethers.Contract(EnvConstant.contractAddress, NFTicketizeContractArt.abi, provider);

    //Insert into smart contract
    const signer = provider.getSigner()
    const daiWithSigner = daiContract.connect(signer);
    var x = await daiWithSigner.createEvent(33333333);
