...
// IPFS initialization
const IPFSComponent = require('ipfs-core');
const ipfs = await IPFSComponent.create();
...
function ButtonsBox({ onReloadList, onSetOverlay, onSetSnackBarMessage}) {
  ...
  const createEvent = async () => {
    ...
    try {
      var result = await nftTicketizeContractWithSigner.createEvent(startTimeMilliseconds, cid.string);
      console.log(result);
    }
    catch (error) {
