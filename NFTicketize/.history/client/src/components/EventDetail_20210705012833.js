  ...
  const buyDialogHandleBuy = async() => {
    ...
    try {
      for (var counter = 0; counter < parseInt(totalNumberOfTickets); counter++) {
        ...
        const pinataApiKey = EnvConstant.pinataURL.pinataHeader[0].pinata_api_key;
        const pinataSecretApiKey = EnvConstant.pinataURL.pinataHeader[0].pinata_secret_api_key;
        const url1 = EnvConstant.pinataURL + "/pinFileToIPFS";
        var x = await axios.post(url1, file, {
          headers: {
            "Content-Type": `multipart/form-data; boundary= ${file._boundary}`,
            pinata_api_key: pinataSecretApiKey,
