...
function TicketCategoryDetail({onReloadList, onSetOverlay, ipfs}) {  
  ...
  const sellDialogHandleSell  = async() => {
    ...
    const orderJSON = createOrder(address, EnvConstant.contractAddress, tokenForDialog, "1000000000000000000");
    const msg = await axios.post("https://api-staging.rarible.com/protocol/v0.1/ethereum/order/encoder/order", orderJSON);
