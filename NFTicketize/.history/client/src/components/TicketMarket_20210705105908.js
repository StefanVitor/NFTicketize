...
function TicketMarketDetail({onReloadList, onSetOverlay, onSetSnackBarMessage, eventKey}) {  
  ...
  const getTickets = async () => {
    ...
    // Fill additional informations for tickets (information about eventName and ticketCategory)
  ...
  const getMyTickets = async (_address, _continuation) => {
    ...
    for(var counter = 0; counter < ticketsDataItems.length; counter++) {
      ...
      // Get information about is ticket for bid from TheGraph
      const ticketQuery = ` {
  ...
  const getBids = async ({ticket}) => {
    if (ticket !== "-1") {
      ...
      for(var counter = 0; counter < bids.length; counter++) {
        ...
        if (make.assetType.assetClass === "ERC721" || make.assetType.assetClass === "ERC721_LAZY") {
  ...
  const acceptBidOffer = async(bid) => {
                                    ...
                                    {bidsData.map(bid=> (
                                      <Grid item xs key={bid.hash}>
                                            <Card className={classes.root}>
                                              <CardContent>
                                                <Typography variant="body2" component="p" color="textSecondary">
                                                  #Bid type - {(bid.type === "ERC721" ||  bid.type === "ERC721_LAZY")? "Ticket": "Currency"}
                                                <br />
                                                </Typography>
                                                <Box component="div" visibility={(bid.type === "ERC721" ||  bid.type === "ERC721_LAZY")? "visible" : "hidden"}>
                                                  <Typography variant="body2" component="p" color="textSecondary">
                                              ...
                                              <CardActions>
                                                  <Box component="div" visibility={bid.accept_bid === true? "visible" : "hidden"}>
                                                    <Button onClick={() => { acceptBidOffer(bid)}} size="small">Accept bid</Button>
                                                  </Box>
                                                  <Box component="div" visibility={bid.cancel_bid === true? "visible" : "hidden"}>
                                                    <Button onClick={() => { cancelBidOffer(bid)}} size="small">Cancel bid</Button>
                                                  </Box>
                                                </CardActions>
                                            </Card>
                                        </Grid>
