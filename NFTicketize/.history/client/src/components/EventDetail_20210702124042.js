...
import { GetSellOrdersByItem, GetMyTicketsPerEvent, CreateMakerOrder, SignOrderMessage, StringToBoolean } from ".././UtilsData";
      ...
      for (var counter = 0; counter < ticketCategories.length; counter++) {
        ...
        if (ipfsData) {
          ...
          data.push({
            ...
            price: ethers.utils.parseEther(ticketCategories[counter].ticketPrice).toString(),
  ...
  const getMyTickets = async (_address, _continuation) => {
    ...
    for(var counter = 0; counter < ticketsDataItems.length; counter++) {
      ...
      if(ordersByItem.length > 0) {
        ...
        obj.resell_value = ethers.utils.parseEther(price).toString();
