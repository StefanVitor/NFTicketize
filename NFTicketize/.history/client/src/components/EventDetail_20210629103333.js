...
function TicketCategoryDetail({onReloadList, onSetOverlay}) {  
  ...
  const [totalNumberOfTicketsFor, setTotalNumberOfTicketsFor] = React.useState(1);
    ...
    try {
      ...
      const tx = await contract.mintAndTransfer(
        [
          0,
          '',
          [[address, 10000]], // You can assign one or add multiple creators, but the value must total 10000
          [[address, 1000]], // Royalties are set as basis point, so 1000 = 10%. 
          ["0x"]
        ],
        minter,
      );
    ...
    <Dialog open={buyDialogOpen} onClose={() => buyDialogHandleClose()} aria-labelledby="form-dialog-title">
      ...
      <DialogContent>
        ...
        <FormControl className={classes.labelFormControl}>
          <InputLabel id="select-tier-time-limit">Total number of tickets?</InputLabel>
          <Select
