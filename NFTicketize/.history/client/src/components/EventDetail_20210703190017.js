...
import { CreateMakerOrder, CreateBidOrderForCurrency, CreateMakerOrderForTrade} from ".././UtilsData";
                      ...
                      <CardActions>
                        ...
                        <Accordion>
                          ...
                          <AccordionDetails>
                            ...
                            <Async promiseFn={GetBidsByTicket(ticket.tokenId, null)}>
                              <Async.Loading>Loading...</Async.Loading>
                              <Async.Fulfilled>
                                {data => {
                                return (
...
function BidDialog({onReloadList, onSetOverlay, bidDialogOpen, setBidDialogOpen, tokenForDialog}) {
  ...
  const bidDialogHandleBid = async() => {
    ...
    if (bidType === 0) {
      ...
      const signer = Provider.getSigner()
      const address = await signer.getAddress();
      order = CreateMakerOrder(address, EnvConstant.contractAddress, tokenForDialog, priceForBidWei.toString());
    } else if (bidType === 1) {
      ...
      const signer = Provider.getSigner()
      const address = await signer.getAddress();
      order = CreateMakerOrderForTrade(address, EnvConstant.contractAddress, tokenForDialog, ticketForTradeOffer);
    }
