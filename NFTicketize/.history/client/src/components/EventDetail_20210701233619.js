...
function TicketCategoryDetail({onReloadList, onSetOverlay, ipfs}) {  
  ...
  const getMyTicketsDataItems = async (ticketsDataItems) => {
    var data = [];
    for(var counter = 0; counter < ticketsDataItems.length; counter++) {
      if (ticketsDataItems[counter].contract.toLowerCase() === EnvConstant.contractAddress.toLowerCase()) {
        ...
        const ordersByItem = await axios.get(EnvConstant.raribleServer + `protocol/v0.1/ethereum/order/orders/sell/byItem=` + EnvConstant.contractAddress +":" + obj.tokendId + `&size=100`);
        //If there is order in list, user could cancel sell(cancel that order), otherwise, user could sell (set order in list)
        if(ordersByItem.orders.length > 0) {
          obj.sell_button = false;
          obj.cancel_sell_button = true;
          obj.resell_value_flag = true;
          var price = ordersByItem.orders[0].take.value;
          obj.resell_value = ethers.utils.parseUnits(price, "gwei");
        }
        else {
