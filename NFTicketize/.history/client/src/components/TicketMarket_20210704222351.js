  ...
  const cancelSellDialogHandleSell = async() => {
    ...
    if (orderForDialog) {
      ...
      try {
        ...
        var result = await axios.post("https://api-ropsten.rarible.com/marketplace/api/v2/orders/"+orderForDialog.hash+"/prepareCancel");
        console.log(result);
        const sendTransactionResponse = await Provider.send("eth_sendTransaction", [result.data.data]);
        console.log(sendTransactionResponse);
