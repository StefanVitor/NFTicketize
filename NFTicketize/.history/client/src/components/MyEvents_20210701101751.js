...
function EventsDetail() {  
  ...
  const getEventsDetail = async(metadataIpfsCid) =>  {
    const ipfsData = await axios.get(EnvConstant.pinataGetURL+ "/" + metadataIpfsCid);
    if (ipfsData) {
...
function ButtonsBox({ onReloadList, onSetOverlay, onSetSnackBarMessage}) {
  ...
  const createEvent = async () => {
    ...
    const pinataIPFS = await axios.post(url, dataForIpfs, EnvConstant.pinataHeader);
    const pinataIPFSHash = pinataIPFS.data.IpfsHash;
    ...
    try {
      const transactionDetail = await nftTicketizeContractWithSigner.createEvent(startTimeMilliseconds, pinataIPFSHash);
