  ...
  const cancelSellDialogHandleSell = async() => {
    ...
    if (orderForDialog) {
      ...
      try {
        ...
        const cancelOrder = CreateCancelOrder(signer.getAddress(), 
          Asset(ERC721, enc(orderForDialog.make.assetType.contract), orderForDialog.make.assetType.value), ZERO, Asset(ETH, enc(t2.address), takeAmount), 1, 0, 0, ORDER_DATA_V1, encDataLeft)
