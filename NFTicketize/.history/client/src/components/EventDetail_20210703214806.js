...
function TicketCategoryDetail({onReloadList, onSetOverlay, ipfs}) {  
  ...
  //Get tickets for bid
  const getBids = async (_ticketId) => {
    var bids = await GetBidsByTicket(_ticketId, null);
    const signer = Provider.getSigner()
    const address = await signer.getAddress();
    var resultBids = [];
    for(var counter = 0; counter < bids.length; counter++) {
      ...
      resultBid.date = new Date(Date.parse(make.createdAt)).toLocaleString(); 
      ...
      const ticketRarible = await GetRaribleInformationAboutTicket(_ticketId);
      ...
      if (make.maker.toLowerCase() === address.toLowerCase()) {
