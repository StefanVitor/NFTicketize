...
function ButtonsBox({onReloadList, onSetOverlay}) {
  ...
  const createNewTicketCategory = async (categoryName, description, limitedUnlimited, maxSubscriber,
    ...
    try {
      ...
      await nftTicketizeContractWithSigner.createTicketCategory(startTimeMilliseconds, cid.string);
  ...
  const handleCreteNewTicketCategory = () => {
    ...
    createNewTicketCategory(categoryName, description, limitedUnlimited, maxSubscriber,
      price, resellFeeType, resellFeeValue);
    setOpen(false);
  };
