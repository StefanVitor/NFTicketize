...
import { GetSellOrdersByItem, GetMyTickets, GetBidsByTicket, GetDetailsAboutTicket, GetTicketsForBid, GetTicketsForSell, CreateCancelOrder, SignOrderMessage, StringToBoolean, PrepareTx } from ".././UtilsData";
...
function TicketCategoryDetail({onReloadList, onSetOverlay, ipfs}) {  
  ...
  //Get tickets for bid
  const getBids = async (_tiketId) => {
    var bids = await GetBidsByTicket(_tiketId, null)
    var resultBids = [];
                      ...
                      <CardActions>
                        ...
                        <Accordion>
                          ...
                          <AccordionDetails>
                            ...
                            <Async promiseFn={getBids(ticket.tokenId)}>
                              <Async.Loading>Loading...</Async.Loading>
                              <Async.Fulfilled>
                                {data => {
                                return (
                                  <Grid container spacing={3} direction='column'>
                                    {data.map(order=> (
                                      <Grid item xs key={order.hash}>
                                            <Card className={classes.root}>
                                              <CardContent>
                                                <Typography variant="body2" component="p" color="textSecondary">
                                                  #Bid type - {ticket.type === "ERC721" ? "Ticket": "Currency"}
                                                <br />
                                                </Typography>
                                                <Box component="div" visibility={ticket.type === "ERC721"? "visible" : "hidden"}>
                                                  <Typography variant="body2" component="p" color="textSecondary">
                                                    #Bid description - description
                                                  <br />
                                                  </Typography>
                                                </Box>
                                                <Box component="div" visibility={ticket.type === "ETH"? "visible" : "hidden"}>
                                                  <Typography variant="body2" component="p" color="textSecondary">
                                                    #Bid value - {ticket.value}
                                                  <br />
                                                  </Typography>
                                                </Box>
                                                <Typography variant="body2" component="p" color="textSecondary">
                                              ...
                                              <CardActions>
                                                  <Box component="div" visibility={ticket.accept_bid === true? "visible" : "hidden"}>
                                                    <Button onClick={() => { buyTicketDialogHandleOpen(ticket.tokenId, ticket.order)}} size="small" >Buy</Button>
                                                  </Box>
                                                  <Box component="div" visibility={ticket.cancel_bid === true? "visible" : "hidden"}>
                                                    <Button onClick={() => { sellDialogHandleOpen(ticket.tokenId)}} size="small" >Sell</Button>
                                                  </Box>
                                                </CardActions>
                                            </Card>
