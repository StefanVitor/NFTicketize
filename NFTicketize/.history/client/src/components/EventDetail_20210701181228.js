...
function TicketCategoryDetail({onReloadList, onSetOverlay, ipfs}) {  
  ...
  const getTickets = async () => {
    ...
    var tickets = await getTicketsByOwner(address, null);
  ...
  const getTicketsByOwner = async (_address, _continuation) => {
    var tickets;
    var newTickets;
    if (_continuation == null) {
      ...
      if (itemsByOwner.data.items.length > 0) {
        ...
        newTickets = await getTicketsByOwner(_address, itemsByOwner.data.continuation);
