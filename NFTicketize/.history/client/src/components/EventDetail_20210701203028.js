...
function TicketCategoryDetail({onReloadList, onSetOverlay, ipfs}) {  
  ...
  const getTickets = async () => {
    ...
    } else if (ticketsTypes === TicketsTypesEnum.MY_TICKETS) {
      tickets = await getMyTickets(address, null);
    }
    /*
  ...
  const getMyTickets = async (_address, _continuation) => {
    ...
    if (_continuation == null) {
      ...
      if (itemsByOwner.data.items.length > 0) {
        tickets = getMyTicketsDataItems(itemsByOwner.data.items);
  ...
  const getMyTicketsDataItems = (ticketsDataItems) => {
    var data = [];
      ...
      <Box pt={1}>
        <FormControl component="fieldset">
          ...
          <RadioGroup aria-label="Tickets types" row  name="ti" value={ticketsTypes} onChange={ event => handleChangeTicketsTypes(event.target.value)}>
            ...
            <FormControlLabel value={TicketsTypesEnum.FOR_BUY} control={<Radio />} label="For buy" />
            <FormControlLabel value={TicketsTypesEnum.OFFER_SELL} control={<Radio />} label="Offer - sell" />
            <FormControlLabel value={TicketsTypesEnum.OFFER_BUY} control={<Radio />} label="Offer - buy" />
          </RadioGroup>
        </FormControl>
      </Box>
    </Box>
