...
import { ApolloClientCustom, Provider, CreateIPFSRef} from ".././UtilsData";
...
function BuyDialog({onReloadList, onSetOverlay, eventKey, buyDialogOpen, setBuyDialogOpen,
  ...
  async function generateTokenId(minter) {
    const res = await axios.get(`/protocol/v0.1/ethereum/nft/collections/`+EnvConstant.contractAddress`/generate_token_id?minter=${minter}`)
    return res.data.tokenId
  }
  
  async function putLazyMint(form) {
    const res = await axios.post("/protocol/v0.1/ethereum/nft/mints", form)
    console.log(res);
    return res.data
  }
 ...
 async function createTestLazyMint() {
 ...
 async function testSignAndCreateLazyMint() {
