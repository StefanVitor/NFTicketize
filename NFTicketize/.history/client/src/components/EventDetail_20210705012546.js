  ...
  const buyDialogHandleBuy = async() => {
    ...
    try {
      for (var counter = 0; counter < parseInt(totalNumberOfTickets); counter++) {
        ...
        var file = new File( [blob], 'canvasImage.jpg', { type: 'image/jpeg' } ); fd.append("canvasImage", file); 
        //var fd = new FormData();
        //fd.append("canvasImage", blob);

        const url1 = EnvConstant.pinataURL + "/pinFileToIPFS";
        var x = await axios.post(url1, fd, {
          headers: {
            "Content-Type": `multipart/form-data; boundary= ${fd._boundary}`,
            pinata_api_key: EnvConstant.pinataURL.pinataHeader[0].pinata_api_key,
            pinata_secret_api_key: EnvConstant.pinataURL.pinataHeader[0].pinata_secret_api_key,
          },
        });
        console.log(x);
        ...
        var data = {
          ...
          "image":"ipfs://ipfs/QmPe1W8XiUYdsvdpvgWVwekQ7cDQivRMak3YPX15vxvSJs/image.png",
