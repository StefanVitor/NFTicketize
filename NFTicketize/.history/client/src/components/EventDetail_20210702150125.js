...
function TicketCategoryDetail({onReloadList, onSetOverlay, ipfs}) {  
  ...
  const cancelSellDialogHandleSell = async() => {
    ...
    if (orderForDialog) {
      ...
      try {
        const cancelOrder = CreateCancelOrder(signer.getAddress(), EnvConstant.contractAddress, tokenForDialog, orderForDialog.take.value)
        const transactionDetail = await raribleExchangeContractWithSigner.cancel(orderForDialog);
