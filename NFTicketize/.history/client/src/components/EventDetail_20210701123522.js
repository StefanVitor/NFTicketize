  ...
  const getTicketsByOwnerContinuation = async (_address, _continuation) => {
    ...
    if (_continuation == null) {
      ...
      if (itemsByOwner.data.items.length > 0) {
        ...
        var newTickets = await getTicketsByOwnerContinuation(_address, itemsByOwner.data.continuation);
        if (newTickets.length > 0) {
          tickets.push(getTicketsByOwnerContinuation(_address, itemsByOwner.data.continuation));
        }
        return tickets;
      }
    } else {
