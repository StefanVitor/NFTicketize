...
function TicketCategoryDetail({onReloadList, onSetOverlay, ipfs}) {  
  ...
  const getTickets = async () => {
    ...
    const itemsByOwner =  await axios.get(EnvConstant.raribleServer + `/protocol/v0.1/ethereum/nft/items/byOwner?owner=` + address + `&size=100`);
  ...
  const signTypedDataNew = async(from, data) =>{
      const msgData = JSON.stringify(data);
 
      // @ts-ignore
      return provider.send("eth_signTypedData_v4", [from, msgData]);
  };
  ...
  const signOrderMessage = async(
    ...
    var signature = await signTypedDataNew(account, data);
    return signature;
  }
  ...
  const sellDialogHandleSell  = async() => {
    ...
    const form = createOrder(address, EnvConstant.contractAddress, tokenForDialog, "1000000000000000");
    ...
    msg.domain.chainId = 3;
    const signature = await signOrderMessage(
      msg.struct, msg.types, msg.structType, msg.domain, form.maker
    )

    form.signature = signature;
    const sendSellOrder = await axios.post(EnvConstant.raribleServer +"/protocol/v0.1/ethereum/order/orders", form);
  ...
  const handleChangeTicketsTypes = (event) => {
    ...
    <Box m={2} pt={1}>
      ...
      <Box pt={1}>
        <FormControl component="fieldset">
          ...
          <RadioGroup aria-label="Tickets types" row  name="ti" value={ticketsTypes} onChange={() => handleChangeTicketsTypes()}>
            <FormControlLabel value="my_tickets" control={<Radio />} label="My tickets" />
            <FormControlLabel value="for_buy" control={<Radio />} label="For buy" />
            <FormControlLabel value="for_offer" control={<Radio />} label="For offer" />
          </RadioGroup>
        </FormControl>
      </Box>
    </Box>
