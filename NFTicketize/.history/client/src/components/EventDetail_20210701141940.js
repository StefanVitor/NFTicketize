...
function TicketCategoryDetail({onReloadList, onSetOverlay, ipfs}) {  
 ...
 /* async function sign(provider, order, account, verifyingContract, types) {
    ...
    async function sign (signMessage, account) {
      const data = createTypeData({
  ...
  const sendSellOrder = await axios.post("https://api-staging.rarible.com/protocol/v0.1/ethereum/order/encoder/order", orderJSON);

  //const value = await sign(orderJSONResponse.data.signMessage)
  /*
  const digest = TypedDataUtils.encodeDigest(orderJSONResponse.data.signMessage);
  const signature = signer.signMessage(digest);
  console.log(signature);
    /*
    // Get sell price
