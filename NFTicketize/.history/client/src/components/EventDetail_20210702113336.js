...
function TicketCategoryDetail({onReloadList, onSetOverlay, ipfs}) {  
  ...
  const cancelSellDialogHandleOpen = (tokenKey) =>  {
    setTokenForDialog(tokenKey);
  ...
  const cancelSellDialogHandleSell = async() => {
    onSetOverlay(true);
    const raribleExchangeContract = new ethers.Contract(EnvConstant.contractAddress, NFTicketizeContractArt.abi, Provider);
    const signer = Provider.getSigner()
    const address = await signer.getAddress();
    const raribleExchangeContractWithSigner = raribleExchangeContract.connect(signer);
    try {
      const transactionDetail = await raribleExchangeContractWithSigner.cancelEvent(startTimeMilliseconds, cid.string);
    } catch (error) {
      onReloadList(true);
      onSetOverlay(false);
    }
