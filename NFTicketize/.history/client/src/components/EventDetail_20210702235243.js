...
import { GetSellOrdersByItem, GetMyTickets, GetTicketsForBid, GetTicketsForSell, CreateMakerOrder, CreateCancelOrder, SignOrderMessage, StringToBoolean, PrepareTx } from ".././UtilsData";
...
function TicketCategoryDetail({onReloadList, onSetOverlay, ipfs}) {  
  ...
  const getTicketsForBid = async (_address) => {
    var ticketsForBid = GetTicketsForBid(_address, eventKey);
    for(var counter = 0; counter < ticketsForBid.length; counter++) {
      ticketsForBid[counter].buy_ticket_button = false;
      ticketsForBid[counter].created_date = new Date(Date.parse(ticketsForBid[counter].date)).toLocaleString();
      ticketsForBid[counter].sell_button = false;
      ticketsForBid[counter].cancel_sell_button = false;
      ticketsForBid[counter].resell_value_flag = true;
  ...
  return (<div>
                    ...
                    <Card className={classes.root}>
                      ...
                      <CardActions>
                          ...
                          <Box component="div" visibility={ticket.cancel_for_bid === true? "visible" : "hidden"}>
                            <Button onClick={() => { forBidDialogHandleOpen(ticket.tokenId)}} size="small" >Cancel for bid</Button>
                          </Box>
                        </CardActions>
                    </Card>
                </Grid>
    ...
    <ForBidCancelDialog onReloadList={onReloadList} onSetOverlay={onSetOverlay}
      forBidDialogOpen={forBidDialogOpen} setForBidDialogOpen={setForBidDialogOpen} tokenForDialog={tokenForDialog}>
    </ForBidCancelDialog>
  </div>);
}
...
function ForBidDialog({onReloadList, onSetOverlay, forBidDialogOpen, setForBidDialogOpen, tokenForDialog}) {
  ...
  function ForBidCancelDialog({onReloadList, onSetOverlay, forBidDialogOpen, setForBidDialogOpen, tokenForDialog}) {
