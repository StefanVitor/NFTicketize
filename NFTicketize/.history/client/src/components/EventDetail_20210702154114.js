...
import { GetSellOrdersByItem, GetMyTickets, GetTicketsForSell, CreateMakerOrder, CreateCancelOrder, SignOrderMessage, StringToBoolean } from ".././UtilsData";
import { Asset, AssetType} from ".././UtilsData";
import { ApolloClientCustom, Provider, ERC721, enc, ETH, ZERO} from ".././UtilsData";
...
function TicketCategoryDetail({onReloadList, onSetOverlay, ipfs}) {  
  ...
  const getTicketsForSell = async (_address, _continuation) => {
    ...
    for(var counter = 0; counter < ticketsDataItems.length; counter++) {
      ...
      obj.order = obj;
      obj.for_offer_sell = false;
      obj.for_offer_buy = false;
      data.push(obj);
    }
    return data;
  }
  ...
  const buyTicketDialogHandleOpen = (tokenKey) =>  {
  ...
  const cancelSellDialogHandleSell = async() => {
    ...
    if (orderForDialog) {
      ...
      try {
        ...
        const cancelOrder = CreateCancelOrder(signer.getAddress(), 
          Asset(ERC721, enc(orderForDialog.make.assetType.contract), orderForDialog.make.assetType.value), ZERO, Asset(ERC20, enc(t2.address), takeAmount), 1, 0, 0, ORDER_DATA_V1, encDataLeft)
                    ...
                    <Card className={classes.root}>
                      ...
                      <CardActions>
                          <Box component="div" visibility={ticket.buy_ticket_button === true? "visible" : "hidden"}>
                            <Button onClick={() => { buyTicketDialogHandleOpen(ticket.tokenId)}} size="small" >Buy</Button>
                          </Box>
