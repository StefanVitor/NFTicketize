...
function BuyDialog({onReloadList, onSetOverlay, eventKey, buyDialogOpen, setBuyDialogOpen,
  ...
  async function generateTokenId(minter) {
    const res = await axios.get(EnvConstant.raribleServer +`/protocol/v0.1/ethereum/nft/collections/`+EnvConstant.contractAddress+`/generate_token_id?minter=${minter}`)
