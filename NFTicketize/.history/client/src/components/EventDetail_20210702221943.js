...
function TicketCategoryDetail({onReloadList, onSetOverlay, ipfs}) {  
  ...
  const forBidDialogHandleOpen = (tokenKey, order) =>  {
    setTokenForDialog(tokenKey);
    setOrderForDialog(order);
  ...
  return (<div>
                    ...
                    <Card className={classes.root}>
                      ...
                      <CardActions>
                          ...
                          <Box component="div" visibility={ticket.for_bid === true? "visible" : "hidden"}>
                            <Button onClick={() => { forBidDialogHandleOpen(ticket.tokenId, ticket.order)}} size="small" >For bid</Button>
                          </Box>
                        </CardActions>
                    </Card>
                </Grid>
    ...
    <ForBidDialog onReloadList={onReloadList} onSetOverlay={onSetOverlay} eventKey={eventKey} buyDialogOpen={buyDialogOpen}
      ticketCategoryNameForDialog={ticketCategoryNameForDialog} ticketCategoryKeyForDialog={ticketCategoryKeyForDialog}
      ticketCategoryPriceForDialog={ticketCategoryPriceForDialog}>
    </ForBidDialog>
  </div>);
...
function BuyDialog({onReloadList, onSetOverlay, eventKey, buyDialogOpen, 
  ...
  const [buyDialogOpenInner, setBuyDialogOpenInner] = React.useState(false);
  const [totalNumberOfTickets, setTotalNumberOfTickets] = React.useState(1);

  useEffect(() => { 
    setBuyDialogOpenInner(buyDialogOpen) 
  }, [buyDialogOpen]);
...
function ForBidDialog({onReloadList, onSetOverlay, forBidDialogOpen, tokenForDialog}) {

  const [forBidDialogOpenInner, setForBidDialogOpenInner] = React.useState(false);
  useEffect(() => { 
    setForBidDialogOpenInner(forBidDialogOpen) 
  }, [forBidDialogOpen]);

  // Dialog for bid button - close
  const forBidDialogHandleClose = () => {
    setForBidDialogOpenInner(false);
  };
  ...
  return (<Dialog open={forBidDialogOpenInner} onClose={() => buyTicketDialogHandleClose()} aria-labelledby="form-dialog-title">
    <DialogTitle id="form-dialog-title">Buy ticket</DialogTitle>
    <DialogContent>
      <DialogContentText id="alert-dialog-description">
