...
function TicketCategoryDetail({onReloadList, onSetOverlay}) {  
  ...
  const getTickets = async () => {
    ...
    const itemsByOwner =  await axios.get(`https://api-dev.rarible.com/protocol/v0.1/ethereum/nft/items/byCollection?collection=` + EnvConstant.contractAddress + `&size=100`);
  ...
  const getTicketsContinuation = async (_address, _continuation) => {
  ...
  const getTicketsDataItems = (ticketsDataItems) => {
    ...
    for(var counter = 0; counter < ticketsDataItems.length; counter++) {
      if (ticketsDataItems[counter].contract === EnvConstant.contractAddress) {
        data.push(ticketsDataItems[counter]);
      }
    }
    return data;
  }
