...
import { GetSellOrdersByItem, GetMyTickets, GetTicketsForSell, CreateMakerOrder, SignOrderMessage, StringToBoolean } from ".././UtilsData";
...
function TicketCategoryDetail({onReloadList, onSetOverlay, ipfs}) {  
  ...
  const getMyTickets = async (_address, _continuation) => {
    ...
    for(var counter = 0; counter < ticketsDataItems.length; counter++) {
      ...
      if(ordersByItem.length > 0) {
        ...
        obj.price_in_usd = ordersByItem[0].makePriceUsd;
  ...
  const getTicketsForSell = async (_address, _continuation) => {
    ...
    for(var counter = 0; counter < ticketsDataItems.length; counter++) {
      ...
      obj.date = new Date(Date.parse(obj.lastUpdatedAt)).toLocaleString();
      obj.sell_button = false;
      obj.cancel_sell_button = false;
      obj.resell_value_flag = true;
      ...
      obj.resell_value = ethers.utils.formatEther(price).toString();
      obj.price_in_usd = obj.makePriceUsd;
              ...
              <Grid item xs key={ticket.tokenId}>
                    <Card className={classes.root}>
                      <CardContent>
                        ...
                        <Box component="div" visibility={ticket.resell_ticket_value === false? "visible" : "hidden"}>
                          <Typography variant="body2" component="p" color="textSecondary" visibility={ticket.resell_ticket_value === false? "visible" : "hidden"}>
