 ...
 async function testSignAndCreateLazyMint() {
  ...
  const buyDialogHandleBuy = async() => {
    ...
    for (var counter = 0; counter < parseInt(totalNumberOfTickets); counter++) {
      const resGenerateToken = await axios.get(EnvConstant.raribleServer +`/protocol/v0.1/ethereum/nft/collections/`+EnvConstant.contractAddress+`/generate_token_id?minter=${minter}`)
      
      
      tokensId.push(resGenerateToken.data.tokenId);
    }
    
    const result = await testSignAndCreateLazyMint();
    console.log(result);
    tokensId.push(result.tokenId);
