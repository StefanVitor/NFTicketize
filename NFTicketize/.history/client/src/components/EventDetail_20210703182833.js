...
import { GetSellOrdersByItem, GetMyTickets, GetDetailsAboutTicket, GetTicketsForBid, GetTicketsForSell, CreateMakerOrder, CreateCancelOrder, SignOrderMessage, StringToBoolean, PrepareTx } from ".././UtilsData";
...
function BidDialog({onReloadList, onSetOverlay, bidDialogOpen, setBidDialogOpen, tokenForDialog}) {
  ...
  const colors = [{id:0, key:'value'}, {id:1, key:'value'}];
  ...
  const [ticketForTradeOffer, setTicketForTradeOffer] = React.useState(0);
  ...
  useEffect(() => {
    const getMyTickets = async() =>  {
      ...
      for(var counter = 0; counter < tickets.length; counter++) {
        ...
        innerTickets.push({
          id: tickets.tokenId,
          value: detailsAboutTicket.eventName + " - " + detailsAboutTicket.ticketCategoryName + " #ID" + tickets.tokenId
        })
        //innerTickets.push({});
      }
