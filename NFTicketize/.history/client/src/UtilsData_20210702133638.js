...
export async function GetTicketsForSell(_address, _continuation, _eventId)  {
    ...
    if (_continuation == null) {
      ...
      if (ticketsForSell.orders.length > 0) {
        tickets = await getTicketsForSellFilter(itemsByOwner.data.items, _eventId, _address);
        if (ticketsForSell.orders.continuation) {
      ...
      if (ticketsForSell.orders.length > 0) {
        tickets = await getTicketsForSellFilter(ticketsForSell.orders.items, _eventId, _address);
        if (ticketsForSell.orders.continuation) {
            newTickets = await GetTicketsForSell(_address, ticketsForSell.orders.continuation);
            if (newTickets.length > 0) {
                tickets.push(newTickets);
            }
        }
