...
export async function GetTicketsForBid(_address, _eventId)  {
    ...
    for(var counter = 0; counter < ticketsData.length; counter++) {
        data[counter].tokenId = parseInt(ticketsData[counter].id);
        const ticket = await axios.get(EnvConstant.raribleServer +`/protocol/v0.1/ethereum/nft/items/` +  EnvConstant.contractAddress + `:` + ticketsData[counter].tokenId);
        data.push(ticket);
    }
