...
export async function GetTicketsForBid(_address, _eventId)  {
    ...
    if (_eventId) {
        const ticketQuery = ` {
            tickets(where: { forBid: 0, eventId:"`+ _eventId +`" }) {
                id
            }
        }`;
    ...
    data = dataFromSubGraph.data.tickets;
    for(var counter = 0; counter < data.length; counter++) {
        data[counter].tokenId = parseInt(data[counter].id);
        const ticket = await axios.get(EnvConstant.raribleServer +`/protocol/v0.1/ethereum/nft/items/` +  EnvConstant.contractAddress + `:` + data[counter].tokenId);
    }
