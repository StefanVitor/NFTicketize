...
export async function GetTicketsForSell(_address, _continuation, _eventId)  {
    ...
    var newTickets;
    if (_continuation == null) {
      const itemsByOwner =  await axios.get(EnvConstant.raribleServer +`protocol/v0.1/ethereum/order/orders/sell/byCollection?collection=` + EnvConstant.contractAddress + `&size=100`);
      if (itemsByOwner.data.items.length > 0) {
        tickets = await getMyTicketsPerEventFilter(itemsByOwner.data.items, _eventId);
        if (itemsByOwner.data.continuation) {
            newTickets = await GetTicketsForSell(_address, itemsByOwner.data.continuation);
            if (newTickets.length > 0) {
                tickets.push(newTickets);
            }
        }
      ...
      const itemsByOwner =  await axios.get(EnvConstant.raribleServer +`protocol/v0.1/ethereum/order/orders/sell/byCollection?collection=` +  EnvConstant.contractAddress + `&size=100&continuation=` + _continuation);
      if (itemsByOwner.data.items.length > 0) {
        tickets = await getMyTicketsPerEventFilter(itemsByOwner.data.items, _eventId);
        if (itemsByOwner.data.continuation) {
            newTickets = await GetTicketsForSell(_address, itemsByOwner.data.continuation);
