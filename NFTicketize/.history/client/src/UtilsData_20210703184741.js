...
export async function GetBidsByTicket(_ticketId, _continuation) {
    ...
    if (_continuation == null) {
        // getSellOrdersByCollection from Rarible
        const ticketsForSell =  await axios.get(EnvConstant.raribleServer +`https://api-dev.rarible.com/protocol/v0.1/ethereum/order/orders/bids/byItem?contract` + EnvConstant.contractAddress + `&tokenId=`+ _ticketId +`&size=100`);
        const orders = ticketsForSell.data.orders;
        const continuation = ticketsForSell.data.continuation;
        if (orders.length > 0) {
            tickets = orders;
        ...
        // getSellOrdersByCollection from Rarible
        const ticketsForSell =  await axios.get(EnvConstant.raribleServer +`https://api-dev.rarible.com/protocol/v0.1/ethereum/order/orders/bids/byItem?contract` + EnvConstant.contractAddress + `&tokenId=`+ _ticketId +`&size=100&continuation=` + _continuation);
        const orders = ticketsForSell.data.orders;
        const continuation = ticketsForSell.data.continuation;
        if (orders.length > 0) {
            tickets = orders;
...
export function CreateMakerOrderForTrade (maker, contract, tokenId, tokenIdTake ) {
    return {
      type: "RARIBLE_V2",
      maker: maker,
      make: {
