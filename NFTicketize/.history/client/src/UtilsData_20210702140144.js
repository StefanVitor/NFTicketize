...
async function getTicketsForSellFilter(_ticketsDataItems, _eventId, _address) {
    var data = [];
    for(var counter = 0; counter < _ticketsDataItems.length; counter++) {
        if (_ticketsDataItems[counter].maker.toLowerCase() !== _address) {
            if (_eventId == null) {
                data.push(_ticketsDataItems[counter]);
            } else {
                const tokensQuery = `
...
export async function GetTicketsForSell(_address, _continuation, _eventId)  {
    ...
    if (_continuation == null) {
        ...
        if (ticketsForSell.data.orders.length > 0) {
            tickets = await getTicketsForSellFilter(ticketsForSell.orders, _eventId, _address);
            if (ticketsForSell.continuation) {
