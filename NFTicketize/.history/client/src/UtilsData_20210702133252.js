...
export async function GetTicketsForSell(_address, _continuation, _eventId)  {
    ...
    var newTickets;
    if (_continuation == null) {
      const itemsByOwner =  await axios.get(EnvConstant.raribleServer +`/protocol/v0.1/ethereum/nft/items/byColletion?collection=` + EnvConstant.contractAddress + `&size=100`);
      if (itemsByOwner.data.items.length > 0) {
