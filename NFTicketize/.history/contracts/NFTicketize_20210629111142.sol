...
contract NFTicketize is ERC721, RoyaltiesV2Impl {
    ...
    function mintAndTransfer(
        ...
        if (_ticketCategory.resellTicketValue > 0) {
            ...
            _royalty.value = 10000 * _ticketCategory.resellTicketValue / 100;
            _royalties.push(royalty);
        }

        for (uint counter = 0; counter < _tokenIds.length; counter++) {
            _mint(_to, _tokenIds[counter]);
            _saveRoyalties(_tokenIds[counter], _royalties);
            ...
            tickets[_tokenIds[counter)] = _ticket;

            emit NewTicket(_eventId, _ticketCategoryId, _tokenIds[counter], 0, 0);
        }
    }
    ...
    function transferFromOrMint(
        ...
        if (_exists(_data.tokenId)) {
            ...
            this.mintAndTransfer(_data, _to, _eventId, _ticketCategoryId);
        }
    }
