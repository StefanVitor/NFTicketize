...
contract("NFTicketize", accounts => {
  ...
  it("...should store correct value on event.", async () => {
    ...
    await NFTicketizeInstance.createTicketCategory(0, 10, uint256 _ticketPrice, uint32 _resellTicketType, uint256 _resellTicketValue, string memory _metadataIpfsCid) public {
    // Get stored value
    //const storedData = await simpleStorageInstance.get.call();
    const storedData = await NFTicketizeInstance.events.call(0);
    console.log(storedData);
