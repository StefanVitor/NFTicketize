...
contract("NFTicketize", accounts => {
  ...
  it("...should check royalty correct set", async () => {
    ...
    await NFTicketizeInstance.mintAndTransfer([1, "-", creators([account_two]), [{account: account_two, value:1000}], [zeroWord]], account_two);

    await NFTicketizeInstance.fillInformationsAboutTicket(eventId, ticketCategory, [1], {from : accounts[1], value:1});

    // Get stored value rolaty
    const storedData = await NFTicketizeInstance.getRoyalties.call(1);
    console.log(storedData);
  ...
  it("should send coin correctly", async () => {
    ...
    await NFTicketizeInstance.createTicketCategory(eventId, 10, price, royalty, "-");
    ...
    await NFTicketizeInstance.mintAndTransfer([2, "-", creators([account_two]), [{account: account_two, value:1000}], [zeroWord]], account_two);

    var result = await NFTicketizeInstance.fillInformationsAboutTicket(eventId, ticketCategory, [2], overrides);
