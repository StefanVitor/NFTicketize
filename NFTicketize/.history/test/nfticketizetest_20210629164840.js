...
contract("NFTicketize", accounts => {
  ...
  it("...should mint correct value", async () => {
    ...
    var eventId = 0;
    var royalty = 10;
    await NFTicketizeInstance.createTicketCategory(eventId, 10, 100, 0, royalty, "-");

    var ticketCategory = 1;
    await NFTicketizeInstance.customMint(accounts[0], eventId, ticketCategory, [0, 1]);

    // Get stored value
    //const storedData = await simpleStorageInstance.get.call();
    const storedData = await NFTicketizeInstance.royalties.call(0);
    console.log(storedData);
    assert.equal(true, storedData.eventId, "The value was not stored.");
  });
});
