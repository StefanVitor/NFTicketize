...
contract("NFTicketize", accounts => {
  ...
  it("...should store correct value on ticketCategory.", async () => {
    ...
    var eventId = 0;
    await NFTicketizeInstance.createTicketCategory(eventId, 10, 0.2, 0, 10, "-");
    // Get stored value
    //const storedData = await simpleStorageInstance.get.call();
    const storedData = await NFTicketizeInstance.ticketCategories.call(0);
    console.log(storedData);
    assert.equal(eventId, storedData.startDate, "The value was not stored.");
  });
});
