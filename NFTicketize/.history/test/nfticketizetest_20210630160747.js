...
contract("NFTicketize", accounts => {
  ...
  it("should send coin correctly", async () => {
    ...
    account_one_starting_balance = await web3.eth.getBalance(accounts[0]);
    account_two_starting_balance = await web3.eth.getBalance(accounts[1]);
    console.log("Account_one balance_before" + account_one_starting_balance);
    console.log("Account_two balance_before" + account_two_starting_balance);
    ...
    var ticketCategory = 1;
    await NFTicketizeInstance.customMint(accounts[0], eventId, ticketCategory, [2, 3], overrides);

    account_one_ending_balance = await web3.eth.getBalance(accounts[0]);
    account_two_ending_balance = await web3.eth.getBalance(accounts[1]);
